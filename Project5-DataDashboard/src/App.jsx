          ...
          <div className="summaries">
            ...
            <div className="summariesCont"> 
              <div className="currSummary">
                ...
                <h3> {fetchResult !== undefined ? Math.floor(fetchResult.reduce((acc, day) => acc + (day.temp || 0), 0) / fetchResult.length) + " °F" : "N/A"} </h3>
              </div>
              <div className="currSummary">
                <h2> Average Relative Humidity </h2>
                <h3> {fetchResult !== undefined ? Math.floor(fetchResult.reduce((acc, day) => acc + (day.rh || 0), 0) / fetchResult.length) + "%" : "N/A"} </h3>
              </div>
              <div className="currSummary">
                <h2> Average Pressure </h2>
                <h3> {fetchResult !== undefined ? Math.floor(fetchResult.reduce((acc, day) => acc + (day.pres || 0), 0) / fetchResult.length) + " mb" : "N/A"} </h3>
              </div>
            </div>
          </div>
        </div>
        <div className="mainFunc"> 
          <Weather dates={fetchResult !== undefined ? fetchResult.map((data, index) => (<p key={index}>{data.datetime} </p>)).reverse() : "N/A"} 
                   temps={fetchResult !== undefined ? fetchResult.map((data, index) => (<p key={index}>{data.temp + " °F"} </p>)).reverse() : "N/A"} 
                   humidities={fetchResult !== undefined ? fetchResult.map((data, index) => (<p key={index}>{data.rh + "%"} </p>)).reverse() : "N/A"} 
                   pressures={fetchResult !== undefined ? fetchResult.map((data, index) => (<p key={index}>{data.pres + " mb"} </p>)).reverse() : "N/A"}
          />
        </div>
      </div>
    </div>
